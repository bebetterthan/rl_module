...
def evaluate_baselines(eval_env, n_episodes=20, verbose=True):
    ...
    if verbose:
        print("\n[Phase1A Wrapper Agent] (smart heuristic)...")
...
def main():
    ...
    print(f"   Action space: 9 discrete (3 tools x 3 modes)")
    ...
    print("   [CHECK] Checkpoints every 25k steps")
    print("   [CHECK] Evaluation every 10k steps")
    print("   [CHECK] Custom Phase 1B metrics enabled")
    ...
    print("\n[Expected duration]: 10-15 minutes (EXTENDED training!)")
    print("="*70 + "\n")
    
    try:
        model.learn(
            total_timesteps=300000,  # DOUBLED from 150k for better convergence!
            callback=callbacks,
            progress_bar=True
        )
    ...
    if success:
        ...
        print("\n[INFO] If test evaluation passes -> READY FOR PHASE 2!")
    else:
        print("\n[WARNING] Training did not meet success criteria")
        print("\n[ITERATE] Options:")
